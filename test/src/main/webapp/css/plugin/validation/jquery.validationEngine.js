...
//					var fixleft = first_err.offset().left;
		...
		*/
		_required: function(field, rules, i, options) {
			switch (field.prop("type")) {
				...
				case "file":
				default:
					if(",mShipname,shipName,".indexOf(","+field.attr("name")+",") != -1){
						if(field.val() == "中文船名"){
							return options.allrules[rules[i]].alertText;
						}
					}
					if(",eshipName,".indexOf(","+field.attr("name")+",") != -1){
						if(field.val() == "英文船名"){
							return options.allrules[rules[i]].alertText;
						}
					}
		...
		*/
		_buildPrompt: function(field, promptText, type, ajaxed, options) {
			...
			if(field.attr("prompt")){
				promptText = field.attr("prompt");
			}
		...
		*/
		_calculatePosition: function (field, promptElmt, options) {
			...
			switch (positionType) {
				...
				case "bottomLeft":
					promptTopPosition = fieldTop + field.height() - 17;
					marginTopSize = 0;
					promptleftPosition = fieldLeft+20;
					break;
				case "bottomRight":
					promptleftPosition = fieldLeft + fieldWidth - 30;
